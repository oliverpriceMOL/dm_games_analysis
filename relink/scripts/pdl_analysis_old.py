...
for d in overlap_dates:
    ...
    for p in pp:
        ...
        # Position 0: time from level_started to first correct guess
        # Fall back to first guess timestamp if level_started not available
...
# Transparency = normalized AUC: sum(errors) / (4 * errors[0])
# <1 means errors decreased (theme helped); naturally rewards early drops
transparency_scores = {}
for d in overlap_dates:
    ds = date_summaries[d]
    pf = pdl_puzzle_features[ds['lid']]
    ...
    for p in pp:
        ...
        # Build solving order: which rows were solved and in what sequence
        seen_rows = set()
        correct_order = []
        ...
        # For each solved row, count wrong guesses for that row before the correct one
    ...
    # Compute mean errors at each position
    pos_means = {}
    pos_ns = {}
    for pos in range(4):
    ...
    # Transparency = center of mass of error curve
    # sum(i * e_i) / sum(e_i) for positions 0-3
    # Midpoint = 1.5 (flat). Below 1.5 = errors front-loaded (transparent). Above = back-loaded (opaque).
...
# Vertical inference data: per-puzzle timing curve + error curve, cross-tabbed by all PDL features
vi_puzzle_data = []
...
for d in overlap_dates:
    ...
    # Timing curve: median inter-correct interval at each position (0, 1, 2, 3)
    ...
    # Timing center of mass: sum(i * t_i) / sum(t_i)
    # Below 1.5 = time front-loaded (players speed up). Above = back-loaded.
...
<h2>Vertical Inference</h2>
<p style="color:var(--muted);margin-bottom:15px;">How do speed and accuracy change as players solve more rows? Both metrics use center of mass (CoM): where does the weight concentrate? Below 1.5 = front-loaded (faster/more errors early = stronger inference). Above 1.5 = back-loaded.</p>
<div class="card" style="margin-bottom:20px;">
    <h3>Summary</h3>
    <div id="vi-summary"></div>
</div>
<div class="card" style="margin-bottom:20px;">
    <h3>Vertical Inference Curves by PDL Feature</h3>
    <p style="color:var(--muted);font-size:13px;margin-bottom:10px;">
        Each feature axis shows two panels: timing curve (median seconds between correct guesses) and error curve (mean wrong guesses per row). A downward curve = stronger vertical inference.
    </p>
    <div id="vi-curve-charts"></div>
</div>
<div class="card">
    <h3>Per-Puzzle Detail</h3>
    <table id="vi-puzzle-table">
        <thead><tr><th>Puzzle</th><th>Date</th><th>Timing Curve</th><th>Timing CoM</th><th>Error Curve</th><th>Error CoM</th><th>Solve Rate</th></tr></thead>
        <tbody></tbody>
    </table>
</div>
</div>
...
(function() {{
    ...
    const curveColors = ['#6c5ce7', '#00b894', '#d63031', '#fdcb6e', '#0984e3', '#e17055', '#00cec9', '#636e72'];
    ...
    features.forEach((feat, fi) => {{
        ...
        if (tcEl) {{
            const datasets = cats.map((cat, ci) => {{
                ...
                const color = curveColors[ci % curveColors.length];
    ...
    function drawSparkline(canvasId, vals, nPoints, isDown) {{
