...
export const TERMS = [
    {
        ...
        terms: [
            ...
            { name: 'Solve Order', page: 'difficulty', definition: 'For each row, the position at which players resolved it within their puzzle attempt — 1st, 2nd, 3rd, or 4th of the four rows. Distinct from "first-try": a row solved 1st might still have taken three wrong guesses to crack. Captures which rows act as entry points (typically solved early) vs deferred (saved for last).' },
            { name: 'Ever / Never Solved', page: 'difficulty', definition: 'For a given row, did the player resolve it at any point during the attempt (ever solved) or did they exhaust their lives before getting to it (never solved)? Tied directly to game progression — players need all 4 rows ever-solved to reach the relink phase. The "never" bucket dominates on the hardest puzzles (e.g., Apr 13: ~51% of rows never solved) and is small on the easiest (~14% on Mar 31).' },
    ...
    },
    {
        ...
        terms: [
            ...
            { name: 'Knowledge Domain', page: 'difficulty', definition: 'The subject area a row draws from — e.g., Music, Sport, Science, Food. PDL v2 allows rows to span multiple domains (a row tagged Language + Geography contributes to both buckets via set intersection, not first-element only). Rows from specialist domains tend to be harder.' },
            { name: 'Real Identity Domain', page: 'difficulty', definition: 'PDL v2 field on each row capturing the domain the impostor word actually belongs to (vs the row\'s domain). E.g. row "fortifications" with impostor "Songbird" → realIdentityDomain=Music. Used to compute the same-domain flag and the new group-domain × impostor-real-domain breakdown.' },
            { name: 'Cross-Domain Impostor', page: 'difficulty', definition: 'A row where the impostor\'s realIdentityDomain has no overlap with the row\'s knowledgeDomain. ~58% of rows on current data. Inverse of same-domain; surfaced explicitly so charts read "cross-domain" rather than "same=False".' },
    ...
    },
    {
        ...
        terms: [
            ...
            { name: 'Domain Mismatch', page: 'published', definition: 'Difficulty dimension measuring whether the impostor comes from a different knowledge domain to the group. A same-domain impostor is more deceptive. Blends wrong rates with the PDL same_domain flag — computed via set intersection between the row\'s knowledgeDomain (multi-valued) and the impostor\'s realIdentityDomain (multi-valued); any overlap = same-domain. Weight: 10%.' },
