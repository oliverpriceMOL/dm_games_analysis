...
- **Relink**: Solve rates vary across 7 puzzles (May 7–13, 22–68%). ~102K total completions (all users), ~65K device-ID trajectories. The full-feature Monte Carlo simulator uses a ratio-shift model stacking adjustments for manipulation, abstraction, knowledge, same_domain, position (imposters phase) and identification manipulation, construction knowledge, tile count (relink phase). Empirical mode: r=0.993, MAE=8.1pp. Predicts solve rates for 41 puzzles without player data. A 5-axis difficulty rating system (manipulation 40%, abstraction 30%, domain mismatch 10%, knowledge 10%, relink challenge 10%) produces 1–5 star ratings with severity-based scoring (avg_wrong/3) and a vertical inference discount. Weights are grid-search-derived with a 10% floor; re-derive via `relink/scripts/_derive_weights_v2.py`.
