...
- `relink/save-data/l{id}.json` — Per-puzzle design file. Contains 4 rows (each with tiles, impostor flags, PDL metadata: manipulation, abstraction, knowledge, knowledgeDomain), relink answer + meta-connection PDL, decoy groupings, board metadata (`phase2TileCount`, `specialistGroupCount`, `isThemed`).
- 20 of the 39 puzzles have matching player behaviour data; the remaining 19 are "undated" (design-only).
...
│   │   │   ├── model.py              # IPW, transitions, simulator
...
│   │   ├── index.html
│   │   ├── css/
│   │   └── js/                       # ES modules: main.js + 13 section renderers
│   ├── outputs/
...
| `relink.json` | Relink phase | Phase 2 by meta-connection type and tile count |
| `clustering.json` | k-means | Puzzle archetypes (k=3) and row archetypes (k=4) |
| `predictions.json` | Difficulty prediction | Predicted vs actual solve rates for all 39 puzzles |
...
- **`pdl_rows`**: Row-level PDL joined across all puzzles (manipulation, abstraction, knowledge, domain, same_domain flag).
- **`date_summaries`**: Per-date aggregated stats, row metrics, timing curves.

### Running

```bash
# Generate all 14 JSON data files (~2-3 min, CSV loading dominates):
...
- **Relink**: Solve rates vary widely across 20 puzzle dates (17%–82%). Mar 31 had the highest solve rate (82%); Apr 9 (21%) and Apr 13 (17%) were the hardest. Player counts grew from ~40/day early on to ~100/day by Apr 10, then tapered. Tutorial usage declined over time (67% → 4%), indicating a maturing player base. Monte Carlo simulator achieves r=0.84 / MAE=18pp on dated puzzles and predicts 59–77% solve rates for undated puzzles.
