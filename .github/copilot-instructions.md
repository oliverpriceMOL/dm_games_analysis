...
│   │   │   ├── metrics.py            # 8 analysis compute functions
│   │   │   ├── model.py              # IPW, transitions, full-feature simulator
│   │   │   └── stats.py              # Stats utilities (OLS, k-means, correlations)
│   │   ├── pdl_analysis.py           # Main pipeline → 13 JSON files + HTML dashboard
...
│   │   └── 05-dashboard.md
│   ├── outputs/
│   │   ├── data/                     # 13 JSON files (generated by pdl_analysis.py)
...
The main analysis is `relink/scripts/pdl_analysis.py`, which cross-references puzzle design parameters (PDL) with player behaviour data. It produces 13 JSON data files consumed by an interactive dashboard.
...
Outputs: 13 JSON files → relink/outputs/data/
...
```bash
# Generate all 13 JSON data files (~5-8 min, CSV loading dominates):
